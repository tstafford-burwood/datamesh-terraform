...
module "bridge_service_perimeter_1" {
  ...
  perimeter_name = format("%s_bridge_foundation_%s", var.prefix, lower(replace(var.researcher_workspace_name, "-", "")))
...
module "bridge_service_perimeter_2" {
  ...
  source         = "terraform-google-modules/vpc-service-controls/google//modules/bridge_service_perimeter"
  policy         = local.parent_access_policy_id
  perimeter_name = format("%s_bridge_image_prj_%s", var.prefix, lower(replace(var.researcher_workspace_name, "-", "")))
