resource "google_storage_bucket_object" "dag" {
  ...
  name   = "dags/${local.environment[terraform.workspace]}_${local.researcher_workspace_name}_workspace_dag.py"
  source = local_file.workspace_dag_py.filename
}

resource "local_file" "workspace_dag_py" {
  filename = "${path.module}/scripts/${local.environment[terraform.workspace]}_${local.researcher_workspace_name}_workspace_dag.py"
...
resource "google_storage_bucket_object" "dataops_delete_dag" {
  # Upload DAG to the data-ops cloud composer DAG bucket.
  bucket = trimprefix(local.dag_bucket, "gs://")
  name   = "dags/${local.environment[terraform.workspace]}_${local.researcher_workspace_name}_dataops_delete.py"
  source = local_file.dataops_delete_dag_py.filename
}

resource "local_file" "dataops_delete_dag_py" {
  filename = "${path.module}/scripts/${local.environment[terraform.workspace]}_${local.researcher_workspace_name}_dataops_delete.py"
